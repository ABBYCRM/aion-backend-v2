...
def all_specs() -> list[SkillSpec]:
    return [
        SkillSpec(id="web.search", name="Web search",
            ...
            side_effect="network",
            input_schema={"type": "object", "required": ["query"], "properties": {"query": {"type": "string"}, "count": {"type": "integer", "default": 5, "minimum": 1, "maximum": 10}}},
        ...
        SkillSpec(id="gdy.meta_catalog_search", name="GDY meta-catalog (local backfill)",
            description="Search the local AION-curated meta-catalog (data/gdy_meta_catalog.json) of 70+ AI coding / RAG / MCP / agent-framework repos from the operator's ai-coding-rag-skills-github-directory.md. Use as fallback when live GDY search returns 0 hits, OR when the user asks for a specific well-known repo like 'context7', 'superpowers', 'docling', 'qdrant' that GDY does not yet index. Each hit returns repo, name, section, best_for, mcp, primary_language, in_gdy.",
            side_effect="read",
        ...
        # ---- v2.8.17 — Hedra (hedra.com) v3 API: unified 30+ image + 20+ video models ----
        SkillSpec(id="hedra.image", name="Hedra image gen (v3 API, 18+ models)",
            description="Generate an image via Hedra v3 API (https://api.hedra.com/v3/models/<slug>). Models: nano-banana-2, nano-banana-pro, nano-banana, imagen4, seedream-5.0-lite, seedream-4.5, seedream-4.0, dreamina-3.1, gpt-image-2-high/medium/low, gpt-image-1.5, flux-2-flex/max/pro, flux-kontext-max/pro, flux-1.1-ultra/pro. Default: nano-banana-2. Auth: `Authorization: Key <key_id>:<secret>`. Operator pinned 2026-08-12: 'I dont want open ai image gen I want hedra'.",
            side_effect="network",
            input_schema={"type": "object", "required": ["prompt"], "properties": {"prompt": {"type": "string", "minLength": 1, "maxLength": 8000}, "model": {"type": "string", "default": "nano-banana-2"}, "aspect_ratio": {"type": "string", "enum": ["1:1", "16:9", "9:16", "4:3", "3:4"], "default": "1:1"}, "n": {"type": "integer", "default": 1, "minimum": 1, "maximum": 4}, "reference_url": {"type": "string"}, "poll": {"type": "boolean", "default": true}, "poll_timeout_seconds": {"type": "integer", "default": 120, "minimum": 10, "maximum": 600}}},
            executor="builtin:hedra.image", tags=["hedra", "image", "v3", "text-to-image"],
            error_codes=["hedra_not_configured", "hedra_auth", "hedra_upstream", "hedra_rejected", "hedra_no_job_id", "hedra_poll_timeout", "missing_required:prompt"]),
        SkillSpec(id="hedra.video", name="Hedra video gen (v3 API, t2v + i2v, 11+ models)",
            description="Generate a video via Hedra v3 API. Models: kling-v3, kling-2.5-turbo, veo-3.1, sora-2-pro, grok-imagine, hedra-avatar, omnia, seedance-2.0, seedance-1.5-pro, omnihuman-1.5, seedance-2.5. Text-to-video + image-to-video (set start_image_url). Auth: `Authorization: Key <key_id>:<secret>`. The xAI Grok Imagine path in lib/grok_reel.py stays — operator pinned that for cartoony identity-locked content.",
            side_effect="network",
            input_schema={"type": "object", "required": ["prompt"], "properties": {"prompt": {"type": "string", "minLength": 1, "maxLength": 8000}, "model": {"type": "string", "default": "kling-v3"}, "start_image_url": {"type": "string"}, "aspect_ratio": {"type": "string", "enum": ["1:1", "16:9", "9:16", "4:3", "3:4"], "default": "16:9"}, "duration_ms": {"type": "integer", "default": 5000, "minimum": 1000, "maximum": 30000}, "quality": {"type": "string", "enum": ["draft", "standard", "pro"], "default": "standard"}, "poll": {"type": "boolean", "default": true}, "poll_timeout_seconds": {"type": "integer", "default": 180, "minimum": 10, "maximum": 600}}},
            executor="builtin:hedra.video", tags=["hedra", "video", "v3", "text-to-video", "image-to-video"],
            error_codes=["hedra_not_configured", "hedra_auth", "hedra_upstream", "hedra_rejected", "hedra_no_job_id", "hedra_poll_timeout", "missing_required:prompt"]),
        ...
        SkillSpec(id="stack.policy.match", name="AION stack policy match",
            ...
            input_schema={"type": "object", "required": ["trigger"], "properties": {
                ...
                "min_score": {"type": "number", "default": 1.25, "minimum": 0, "maximum": 10},
            }},
...
def wire_executors() -> None:
    ...
    r.register_many({
        ...
        "builtin:hedra.image_to_video": hedra.hedra_image_to_video,
        "builtin:hedra.image": hedra.hedra_image,
        "builtin:hedra.video": hedra.hedra_video,
