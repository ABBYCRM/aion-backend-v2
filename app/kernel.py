...
- pypi.tier1_search: search the operator's local tier1_pypi catalog (data/gdy_meta_catalog.json tier1_pyi section, 14 entries from the 2026-08-12 gift batch: fastapi-rag, browser-automation-cli, exocortex-llm-router, agentpack-skills, agent-tool-router, semantic-tool-router, etc.). Use this BEFORE web.search when the operator asks for a specific tool by name from that batch — it returns the AION-relevance score + install status from the live install log.
- hedra.image: generate an image via Hedra v3 API (https://api.hedra.com/v3/models/<slug>). 18+ image models including nano-banana-2 (default, Google character-consistency), imagen4, seedream-5.0/4.5/4.0, gpt-image-2-high/medium/low, flux-2-flex/max/pro, flux-kontext-max/pro. The image generate route at /api/image/generate defaults to hedra-nano-banana-2 — use gpt-image-* explicitly if you want OpenAI.
- hedra.video: generate a video via Hedra v3 API. 11+ video models including kling-v3 (default), veo-3.1, sora-2-pro, grok-imagine, hedra-avatar, omnia, seedance-2.0/1.5-pro, omnihuman-1.5. Text-to-video + image-to-video (set start_image_url). The xAI Grok Imagine path in lib/grok_reel.py stays — operator pinned it for cartoony identity-locked content (the teeth-duck fix).
- Model defaults v2.8.17: openai_chat_model=gpt-4.1 (not mini, not gpt-4o), openai_tts_model=gpt-4o-tts, primary_model=openai/gpt-4.1. The operator explicitly said "I don't want a shitty open ai model for my main chat".
